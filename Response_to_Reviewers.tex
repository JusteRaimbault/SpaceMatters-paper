...
B. To highlight limitations as well as promising perspectives of improvement, we answer the concern about the genericity of the method and expose its links with the conceptual and empirical domains:
 ...
 \begin{itemize}
  ...
  \item B.1. We have added a discussion of\comment[FL]{on?} how segregation indicators can themselves be linked with geometric structure (which can typically happens if an indicator give the same weight to cells of different densities). This is a limitation of the method as it is but also an avenue for promoting stronger indicators in empirical research. % \comment[MLT]{j'ai formulé un paragraphe court sur ce point en discussion, en lien avec le point B.3}
  ...
  \item B.3. As a perspective to reduce this bias, we could compute ego-neighbourhoods rather than a regular grid (another paper)\comment[FL]{pas clair : pour calculer les indicateurs? pour les interactions? à préciser comment sont utilisés ces ego neighborhoods}. We could also\comment[FL]{plutôt : In further researches, we plan to extend the analysis...} in another paper vary the size of grid and its cell shape (cf. MAUP), but also generate other types of geometric structures to feed into geosimulation models (for example points or networks). % \comment{j'ai formulé un paragraphe court sur ce point en discussion, en lien avec le point B.1}
\end{itemize}
...
%\textbf{to do list:}
 ...
 % \textbf{Sociospatial system} pour parler des systèmes empiriques, \textbf{interactions} pour parler de ce qui se passe dans le modele,  \textbf{geometric structures} pour parler des outputs issus du générateur et  parler de \textbf{density grid generator} quand on développe le nôtre. \comment{done}
 
 % caser" Jessop (2008) points in particular to the interdependence between social phenomena and the spatial environment where they take place. We model the social system as the interaction dynamics of agents and spatial units. The spatial generator produces the geometric support for these interactions." in intro 1.4 \\ \comment[MLT]{To avoid redundancies, I have simply added the reference to Jessop when we describe Figure 1.}
...
\medskip
 ...
 \medskip
  ...
  $\rightarrow$ see comment A. cite Banos, 2012 \comment[FL]{quel était le point exactement avec Banos?}
