...
\begin{itemize}
    ...
    %\comment[FL]{je ne sais pas si dire "conflict" sera productif?, de même que de parler de "parties"? ci-dessous en commentaires la réponse précédente, j'ai juste reformulé}[(JR) ok nickel]
...
\begin{itemize}
   ...
   %\comment[FL]{je ne suis pas sûr d'être d'accord avec cette réponse : si on est pas en mesure de fournir des illustrations frappantes du truc d'entrée de jeu, c'est dommage. Pas sûr que se cacher derrière les grilles complètes soit le bon argument.}[(JR) du coup on est ok que ca compliquerait trop l'autre Schelling encore et qu'on le met pas ?]
   %\comment[JR]{Possibility to find configuration with an ``eyeball'' difference ? i'm not sure of that..}[(CC)peut-être en prenant des cas extrêmes sur Sugarscape?][(JR) je regarde... $\rightarrow$ bof, on voit rien a part que les agents se repartissent differement mais qui est une consequence directe d'ajouter l'espace, en en comparant les valeurs finales des ginis, ce qu'on fait plus bas plus rigoureusement. je suggère poliment que sa suggestion est idiote.]
	...
	\item There was a misunderstanding for the Schelling implementation actually used, as we used our own implementation which pseudo-code has been added in Appendix of the paper and source code is openly available on the git repository of the project. This version implements the one used by (Gauvin et al., 2009), which also influenced us for the use of the term ``phase diagram''.% It does not reach a segregated state for high values of intolerance, but it does not cause a problem in our case as indicators can be computed on any model state (and actually reflect the fact that it is not segregated).
	 %\comment[CC]{le modèle ne produit pas de ségrégation quand les individus sont très intolérants? Comment ça? Bien sûr qu'il le fait (sous contrainte du taux de vacance). Cela dit y'a bien un point sur lequel l'implem Netlogo n'est pas bonne: c'est que la relocalisation des mécontents est random alors que dans le bouquin de Schelling, apparemment, c'était dans un voisinage proche.}[(JR) ah oui cest selon la vacance ; mais il y a des regimes ou ca converge jamais sous forme de cluster et ca reste un etat dynamique sans structure ; oui par contre on a du reprendre la random reloc, c'est ce que font les physiciens aussi]
...
%: "In an urban environment, Sugarscape can be used to model how the spatial distribution of any type of goods or services can influence the spread of wealth among inhabitants." % pas besoin de mettre les modifs ici
	...
	\item We agree that the term of meta-parameters may have been confusing, and following the suggestion we replaced it with ``generator parameters''.
     %\comment[JR]{TB la def Marion ; là on peut choisir de virer en effet le terme, ou dire que les 3 sont interchangeable mais que "meta-" est plus pratique, qu'en pensez vous ?}
    %\comment[CC]{d'acc.}
    %\comment[MLT]{Du coup, dans nos échanges par mails on disait qu'on était ok pour changer, mais finalement cette solution vous convient? Je comprends pourquoi il/elle a tiqué sur le terme, mais c'est pas non plus hors de propos vu l'application qu'on en fait... bref, je suis partagée!}[(JR) moi ca me va de virer le meta]\comment[FL]{moi aussi ca me va de virer méta}
	...
	\item \comment[JR]{on avait teste avec le Monge ca donnait a peut pres pareil ; donc les normes alpha surement encore plus. je regarde.}\comment[FL]{je pense qu'on peut assumer le côté exploratoire de l'approche : on a deux modèles déjà et on présente un framework : on peut ouvrir en disant qu'ensuite en fonction des indicateurs on pourrait avoir des agrégateurs mathématiques ad hoc, parfois L2, L1, Linf, mais que là pour faire "passe partout" on présente L2 que tout le monde connait? }
	
	% 8. Regarding the spatial generator: the authors should explain why this generator is relevant to these two models? 
	\item This spatial generator is relevant to both model as it produces density grids at a ``metropolitan scale'' (see detailed discussion added in Appendix and the original paper from which we used the generator), which is the scale at which both model were initially intended to be. We made this aspect more explicit. \comment[FL]{c'est léger comme justification : je dirais que ton générateur est crédible il produit des indicateurs dont l'espace de variation est crédible quand on le confronte aux observations empiriques : il faut que tu parles je pense de la ref où tu as les deux nuages de points superposés. Sinon cela fait un peu "cela marche car cela marche"}
    ...
    %(e.g. the least realistic, although this is the case of most Schelling models).\comment[FL]{reformuler en enlevant harmful : simplement "is potentially the most common simplification of the heterogeneity of space, with consequences on the model outputs often not evaluated}
	%\comment[CC]{Ref du papier de batty à ajouter (je ne l'ai pas trouvée}[(c'est le Batty, 2005)]
...
%\comment[CC]{argumentaire à ajouter dans le papier}[(cc) done]\comment[FL]{top}
...
\begin{itemize}
	...
	\item Indeed, we have tried to review articles where some kind of sensitivity to initial spatial conditions are tackled. \comment[FL]{ci dessous en commentaire l'ancienne version, j'ai essayé d'arrondir la formulation ca fait agacé genre "bon j'ai raison : d'abord..., ensuite..."}. However we would like to emphasize that the references cited in this section are few compared to the mass of geosimulation articles published with hardly any mention of dependence of output on initial spatial condition. Moreover, even if such sensitivity is tester, it mostly provide some hints of impact of initial spatial conditions on the output of models (due to the accuracy of data, shape of boundaries and spatial heterogeneity) but to the best of our knowledge no article provide a systematic inquiry into the impact of systematically defined spatial input on the model output in the way we try to provide later on in the paper. We have tried to argue this point at the end of the section.
    %\item Indeed, we have tried to review articles where some kind of sensitivity to initial spatial conditions are tackled. But first\comment[FL]{arrondir la formulation ca fait agacé genre "bon j'ai raison : d'abord..., ensuite..."}, the references cited in this section are few compared to the mass of geosimulation articles published without any mention of dependence of output on initial spatial condition. Second, the references cited mostly provide some hints of impact of initial spatial conditions on the output of models (due to the accuracy of data, shape of boundaries and spatial heterogeneity) but none provide a systematic inquiry into the impact of systematically defined spatial input on the model output in the way we try to provide later on in the paper. We have tried to argue this point at the end of the section.
    %\comment[CC]{argumentaire à ajouter dans le papier}[(cc) done]
...
\begin{itemize}
	...
	\item \comment[JR]{no justification for the difference in number of runs (just experimental setup contingence\ldots ) $\rightarrow$ either new runs or redo computations with 50 for Schelling}\comment[JR]{ca va etre chaud patate pour les new computations ; je vais regarder les ratio de sharpe, avec un peu de chance il y a besoin de plus de repets pour schelling...}
