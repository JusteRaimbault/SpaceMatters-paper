\documentclass[preprint,5p,times,twocolumn,authoryear,draft]{elsarticle}
...
\added{As detailed in supplementary material~\ref{app:convergence}, more repetitions are needed for Schelling indicators than for Sugarscape, in order to obtain a similar relative confidence in the estimation of averages. We ran for this reason a different number of replications for each model.}
...
\added{We give below the pseudo-code for the implementations we used of both sugarscape and schelling models. We recall that source code is openly available at} \url{https://github.com/AnonymousAuthor2/SpaceMatters}. \added{The pseudo-code is in the style of NetLogo code, which is already easily readable.}



%%%%%%%%%%%%%%%
\begin{figure*}
\caption{\added{\textbf{Pseudo-code (NetLogo style) for the Schelling model used here.} We give a pseudo-code very close to the NetLogo language, at the exception of italic expressions which are not put here in NetLogo to ease understandability.}}
\begin{mdframed}
\begin{lstlisting}
...
\caption{\added{\textbf{Pseudo-code (NetLogo style) for the sugarscape model.} Conventions are the same than for Schelling pseudo-code.}}
\begin{mdframed}
\begin{lstlisting}
