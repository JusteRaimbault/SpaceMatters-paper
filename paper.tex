...
where $\mu_{\vec{\alpha}_i}$ are the phase diagrams at given \replaced{generator}{meta-} parameters $\vec{\alpha}_i$. $d$ is a functional distance that we take simply as Euclidian distance. The internal variabilities are estimated as the variance within each phase diagram $f_{\vec{\alpha_i}}$. For a multi-dimensional phase diagram, we \replaced{average}{sum} these relative distances over the components. 
...
For both models we estimate the Sharpe ratios for each indicator by
