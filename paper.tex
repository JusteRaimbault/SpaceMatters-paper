...
\added{For both models we estimate the Sharpe ratios for each indicator by $S(X) = \hat{\mathbb{E}{X}}/\hat{\sigma}(X)$ with standard estimators for average and standard deviation. The summary statistics computed on repetitions for all parameter points are given in Table~\ref{tab:summarysharpes}.}
